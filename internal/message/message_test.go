...
// TestDeleteTurn verifies that DeleteTurn removes exactly one user
// turn: the anchoring user message plus every message up to (but not
// including) the next user message, no matter which message inside the
// turn is used as the anchor.
func TestDeleteTurn(t *testing.T) {
	t.Parallel()

	svc, sessionID := newTestService(t)

	newMsg := func(role MessageRole, content string) Message {
		// Sleep so created_at (ms precision) strictly increases and
		// List's ORDER BY created_at matches insertion order.
		time.Sleep(2 * time.Millisecond)
		msg, err := svc.Create(t.Context(), sessionID, CreateMessageParams{
			Role:  role,
			Parts: []ContentPart{TextContent{Text: content}},
		})
		require.NoError(t, err)
		return msg
	}

	user1 := newMsg(User, "question one")
	assistant1 := newMsg(Assistant, "answer one")
	user2 := newMsg(User, "question two")
	assistant2 := newMsg(Assistant, "answer two")
	user3 := newMsg(User, "question three")
	assistant3 := newMsg(Assistant, "answer three")

	// Anchor on the assistant message of the middle turn: the whole
	// middle turn (user2 + assistant2) must be deleted.
	deleted, err := svc.DeleteTurn(t.Context(), sessionID, assistant2.ID)
	require.NoError(t, err)
	require.Len(t, deleted, 2)
	require.Equal(t, []string{user2.ID, assistant2.ID}, []string{deleted[0].ID, deleted[1].ID})
	...
	require.Equal(t, []string{user1.ID, assistant1.ID, user3.ID, assistant3.ID}, ids)
}

// TestDeleteTurnUnansweredTurn verifies that deleting a trailing user
// message with no reply yet removes only that user message.
func TestDeleteTurnUnansweredTurn(t *testing.T) {
	t.Parallel()
	...
	user1 := newMsg(User, "question one")
	assistant1 := newMsg(Assistant, "answer one")
	user2 := newMsg(User, "question two") // no answer yet

	deleted, err := svc.DeleteTurn(t.Context(), sessionID, user2.ID)
	require.NoError(t, err)
	require.Len(t, deleted, 1)
	require.Equal(t, user2.ID, deleted[0].ID)

	remaining, err := svc.List(t.Context(), sessionID)
	require.NoError(t, err)
	require.Len(t, remaining, 2)
	require.Equal(t, user1.ID, remaining[0].ID)
	require.Equal(t, assistant1.ID, remaining[1].ID)
}

// TestDeleteTurnUnknownAnchor verifies that an unknown anchor message
// ID errors and leaves the session untouched.
func TestDeleteTurnUnknownAnchor(t *testing.T) {
	t.Parallel()

	svc, sessionID := newTestService(t)
	_, err := svc.DeleteTurn(t.Context(), sessionID, "does-not-exist")
	require.Error(t, err)
}
